...
{
    public partial class VendorCustomerListViewForm : BaseListPage
    {
        [Inject] public IVendorCustomerService _VendorCustomerService { get; set; }
        private IEnumerable<VendorCustomerListViewModel>? _items;
        ...
        {
            _items = await _VendorCustomerService.GetListViewItems(_searchText, cancellationToken);
        }
        ...
        {
            if (_selectedItemId == null) return;
            await _VendorCustomerService.DeleteDetailViewModel((Guid)_selectedItemId, cancellationToken);
            await LoadData();
            _selectedItemId = null;
        }
        private async Task HandleDoubleClickRow(GridRowClickEventArgs arg)
        {
            //_selectedItemId = (_dataGrid.SelectedDataItem as VendorCustomerListViewModel)?.Id;
            await HandleNewItem();
        }
