...
{
    public partial class PlaceParameterListViewForm : BaseListPage
    {
        [Inject] public IPlaceParameterService _PlaceParameterService { get; set; }
        private IEnumerable<PlaceParameterListViewModel>? _items;
        ...
        {
            _items = await _PlaceParameterService.GetListViewItems(_searchText, cancellationToken);
        }
        ...
        {
            if (_selectedItemId == null) return;
            await _PlaceParameterService.DeleteDetailViewModel((Guid)_selectedItemId, cancellationToken);
            await LoadData();
            _selectedItemId = null;
        }
        private async Task HandleDoubleClickRow(GridRowClickEventArgs arg)
        {
            //_selectedItemId = (_dataGrid.SelectedDataItem as PlaceParameterListViewModel)?.Id;
            await HandleNewItem();
        }
